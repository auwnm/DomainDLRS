...
for (i in 1:N) {
  ...
  dd_file <- paste0(path,fam[i],"/duplication_loss_sim",fam[i],"_domainDLRS_ZNF.csv");
  mb_file <- paste0(path,fam[i],"/duplication_loss_sim",fam[i],"_MrBayes_ZNF.csv");
  ...
  colnames(df_dd) <- c("Duplication", "Loss", "method","family");
  colnames(df_mb) <- c("Duplication", "Loss", "method", "family");
...
melt_df<-melt(data=sim_znf, id.vars = c("method","family"), measure.vars = c("Duplication", "Loss"))
...
pl = ggplot(plot.df, aes(Family, Mean, fill= Method)) + 
  ...
  geom_errorbar(aes(ymin = Mean - SD, ymax = Mean + SD), width = 0.2, inherit.aes = TRUE,  position=position_dodge(width=0.9))


pl = ggplot(melt_df, aes(x=Family, y=Percentage, color=Method, fill=Method)) + theme_bw() +
      theme(text = element_text(size=16), axis.text.x = element_text(angle=90, vjust=1)) + 
      xlab('Family') + ylab('Normalized score') +
...
pdf( paste0(outPath,"accuracyPlot_SZNF_Domain_duploss_Score.pdf", sep=""));
plot(pl)
dev.off()
