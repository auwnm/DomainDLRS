...
/**
 * Represents a 1-D real valued interval of the type [a,b), (a,b], (-inf,inf),
 * etc.
 * 
...
public class RealInterval {
	 ...
	 * 
	 * @param a
	 *            lower bound, possibly Double.NEGATIVE_INFINITY.
	 * @param b
	 *            upper bound, possibly Double.POSITIVE_INFINITY.
	 * @param isLeftOpen
	 *            true if lower bound is not itself valid, e.g. (a,b].
	 * @param isRightOpen
	 *            true if upper bound is not itself valid, e.g. [a,b).
	 */
	...
	/**
	 * Returns a random double uniformly drawn from this interval. Not extensively
	 * tested, and with several theoretic idiosyncrasies:
	 * <ul>
	 * <li>Cases where the span (b-a) > Double.MAX_VALUE are not supported. This
	 * goes for some similar cases as well, e.g., (a,inf) for finite a < 0.</li>
	 * <li>For [a,b], endpoints will be returned half as often as any other
	 * number.</li>
	 * <li>For (-inf,inf), 0 will be returned twice as often as any other
	 * number.</li>
	 * </ul>
	 * 
	 * @param rng
	 *            a (possibly pseudo) random number generator.
	...
	public double getRandom(Random rng) {
		...
		if ((diff < 0) || ((t == Type.LEFT_OPEN || t == Type.LEFT_CLOSED) && this.a < 0)
		...
		if ((this.isLeftOpen && !Double.isInfinite(this.a)) || (this.isRightOpen && !Double.isInfinite(this.b))) {
			while (d == 0) {
				d = rng.nextDouble();
			}
		}
		...
		case CLOSED:
			...
			return (rng.nextBoolean() ? (up - lo) * d + lo : (up - lo) * (1.0 - d) + lo);
		...
		case UNBOUNDED:
			...
			return (rng.nextBoolean() ? d * Double.MAX_VALUE : -d * Double.MAX_VALUE);
