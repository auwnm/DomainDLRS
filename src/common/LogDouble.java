...
/**
 * Class for floating point numbers where the value is kept in log-form
 * internally. As such, it enables higher precision than a regular double for
 * e.g. very small probabilities 0 <= p << 1, although it can be used for values
 * greater than 1 and negative values as well. This class is called
 * <code>Probability</code> in the original PrIME C++ package.
 * <p/>
 * NOTE: "log" here only refers to the internal representation; one should think
 * of a <code>LogDouble</code> instance as a floating point number like any
 * other.
 * 
...
public final class LogDouble implements Comparable<LogDouble> {
	...
	/**
	 * Log-value, sign of actual value discarded. If the actual value is 0, this may
	 * be set to anything, (although most often it will be set to
	 ...
	 * 
	 * @param d
	 *            the actual value, i.e. non-logged.
	 */
	 ...
	 * 
	 * @param logValue
	 *            the log-value. Of no importance if sign == 0.
	 * @param sign
	 *            the sign: 1 = positive, 0 = zero, -1 = negative.
	 */
	...
	/**
	 * Returns log(|v|) for the actual value v. If v==sign==0, the returned value
	 * may be anything (although commonly Double.NEGATIVE_INFINITY).
	 * 
	...
	/**
	 * Returns the sign of the actual value, where 1 = positive, 0 = zero, -1 =
	 * negative.
	 * 
	 * @return the sign.
	 */
	...
	/**
	 * Returns the actual value (non-logged). There may of course be a loss of
	 * precision (small values may e.g. be rounded to 0).
	 * 
	...
	/**
	 * Adds another LogDouble to this LogDouble. Does not yield a new instance; for
	 * that purpose, see <code>addToNew()</code>.
	 * 
	 * @param q
	 *            the LogDouble to add.
	...
	/**
	 * Adds a double to this LogDouble. Does not yield a new instance; for that
	 * purpose, see <code>addToNew()</code>.
	 * 
	 * @param q
	 *            the double to add.
	 * @return this LogDouble, not a new instance.
	...
	/*
	 * Perform log summation of exponentials when very very small numbers are there.
	 * Please see Ref. 1&2 below. Ref1 : http://jblevins.org/log/log-sum-exp ; Ref2
	 * : http://lingpipe-blog.com/2009/06/25/log-sum-of-exponentials/
	 */
	public static LogDouble logSumOfExp(ArrayList<LogDouble> V) {

		Collections.sort(V);
		LogDouble max = V.get(V.size() - 1);

		double expPart = 0.0;
		for (int i = 0; i < (V.size() - 1); i++)
			expPart += Math.exp(V.get(i).getLogValue() - max.getLogValue());

		double sum = max.getLogValue() + Math.log1p(expPart);
		LogDouble rv = new LogDouble(sum, max.getSign());
	...
	/**
	 * Adds another LogDouble to this LogDouble. Does not yield a new instance; for
	 * that purpose, see <code>subToNew()</code>.
	 * 
	 * @param q
	 *            the LogDouble to subtract.
	 * @return this LogDouble, not a new instance.
	 */
	...
	/**
	 * Adds a double to this LogDouble. Does not yield a new instance; for that
	 * purpose, see <code>subToNew()</code>.
	 * 
	 * @param q
	...
	/**
	 * Produces a new LogDouble instance as this LogDouble minus another. See also
	 * <code>sub()</code>.
	 * 
	 * @param q
	 *            the LogDouble to subtract.
	...
	/**
	 * Produces a new LogDouble instance as this LogDouble minus a double. See also
	 * <code>sub()</code>.
	 * 
	 * @param q
	 *            the double to subtract.
	...
	/**
	 * Multiplies this LogDouble with another LogDouble. Does not yield a new
	 * instance; for that purpose, see the <code>multToNew()</code> method.
	 * 
	 * @param q
	 *            LogDouble to multiply with.
	 * @return this LogDouble, not a new instance.
	 */
	...
	/**
	 * Multiplies this LogDouble with a double. Does not yield a new instance; for
	 * that purpose, see the <code>multToNew()</code> method.
	 * 
	 * @param q
	...
	/**
	 * Produces a new LogDouble instance as this LogDouble times another. See also
	 * <code>mult()</code>.
	 * 
	 * @param q
	 *            the LogDouble to multiply with.
	 * @return a new LogDouble.
	 */
	...
	/**
	 * Produces a new LogDouble instance as this LogDouble times a double. See also
	 * <code>mult()</code>.
	 * 
	 * @param q
	...
	/**
	 * Divides this LogDouble with another LogDouble. Does not yield a new instance;
	 * for that purpose, see the <code>divToNew()</code> method.
	 * 
	 * @param q
	 *            LogDouble to divide with.
	 * @return this LogDouble, not a new instance.
	 */
	...
	/**
	 * Divides this LogDouble with a double. Does not yield a new instance; for that
	 * purpose, see the <code>divToNew()</code> method.
	 * 
	 * @param q
	 *            double to divide with.
	 * @return this LogDouble, not a new instance.
	 */
	...
	/**
	 * Produces a new LogDouble instance as this LogDouble divided by another. See
	 * also <code>div()</code>.
	 * 
	 * @param q
	 *            the LogDouble to divide by.
	 * @return a new LogDouble.
	 */
	...
	/**
	 * Produces a new LogDouble instance as this LogDouble divided by a double. See
	 * also <code>div()</code>.
	 * 
	 * @param q
	 *            the double to divide by.
	 * @return a new LogDouble.
	 */
	...
	/**
	 * Returns a new LogDouble equal to this raised to a power. See also
	 * <code>pow()</code>.
	 * 
	 * @param i
	 *            the power.
	...
	/**
	 * Sets the value of this LogDouble to e^v where v is the old value. See also
	 * <code>expToNew()</code>.
	 * 
	...
	/**
	 * Returns a new LogDouble with value e^v where v is the value of this
	 * LogDouble. See also <code>exp()</code>.
	 * 
	 * @return a new LogDouble.
	 */
	...
	/**
	 * Sets the value of this LogDouble to ln(v) where v is the old value. For
	 * v==sign==0, sets the new value to negative infinity. See also
	 * <code>logToNew()</code> and <code>getLogValue()</code>.
	 * 
	 * @return this LogDouble.
	 */
	...
	/**
	 * Returns a new LogDouble with value ln(v) where v is the value of this
	 * LogDouble. See also <code>log()</code>.
	 * 
	 * @return a new LogDouble.
	 */
	...
	/**
	 * Overridden hash code. Two LogDouble instances are considered equal if they
	...
	/**
	 * Overridden equality method. Two LogDouble instances are considered equal if
	 * they represent the same number. Beware, though, floating point equality
	...
	/**
	 * Helper. Adds a LogDouble to this object, sign issue assumed to be resolved
	 * already.
	 * 
	 * @param q
	 *            LogDouble to add.
	...
	private void add_(LogDouble q) {
		...
		if (this.p > q.p) {
			...
			// Step by step
			// double diff = q.p - this.p;
			// double expDiff = StrictMath.exp(diff);
			// double logExpDiff = StrictMath.log1p(expDiff);
			// this.p = this.p + logExpDiff;

			p = this.p + StrictMath.log1p(Math.exp(q.p - this.p));
		} else {

			// Step by step
			// double diff = this.p - q.p;
			// double expDiff = StrictMath.exp(diff);
			// double logExpDiff = StrictMath.log1p(expDiff);
			// this.p = q.p + logExpDiff;
	...
	/**
	 * Helper. Subtracts a LogDouble from this object, sign issue assumed to be
	 * resolved already.
	 * 
	 * @param q
	 *            LogDouble to subtract.
	 */
	...
	public String toString() {
		...
		case -1:
			return ("neg(" + Double.toString(this.p) + ")");
		default:
			throw new ArithmeticException("Sign of LogDouble instance has illegal value.");
		}
	}
