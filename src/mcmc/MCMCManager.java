...
public class MCMCManager {
	...
	public MCMCManager(Parameters pi, RealParameterUniformPrior edgeRateDistParameterPrior, ProposerSelector ps,
			ProposalAcceptor proposalAcceptor, OutputHandler output, PRNG prng, boolean useRootEdge, Log log) {
		this.ps = ps;
		this.pi = pi;
		this.log = log;
		this.prng = prng;
		...
		do {
			...
			if (doAccept) {
				...
				if (bestStateLikelihood.unnormalizedDensity.lessThan(proposedStateLikelihood.unnormalizedDensity)) {
					...
					for (int r = 0; r < pi.numberOfDomains; r++) {
						this.bestSample.append(
								output.getDomainParametersStr(iteration, pi, r, bestStateLikelihood, true, true));
						this.bestSample.append("\n");
					}
				}
			...
			if ((iteration % TuningParameters.thinningFactor) == 0) {
				...
				if (iteration % (percent * 10.0) == 0 && iteration != 0)
					System.out.print("\n" + String.format("%d", (int) (iteration / percent))
							+ "% completed out of maximum allowable iterations ... ");
			}

			if (this.pa.hasExhausted()) {
				log.write("\nOptimum reached: " + TuningParameters.hillClimbingMaxTries
...
//////////////////////////////////////////////////
/*
 * log.write("\n\nBefore Perturbation: "); log.write(pi.domEdgeRates[0][0] +
 * "\t" + pi.domEdgeRates[0][1] + "\t" + pi.domEdgeRates[1][0] + "\t" +
 * pi.domEdgeRates[1][1] );
 * 
 * for(int r=0; r<pi.numberOfDomains ; r++) { for(int j=0 ; j<2 ; j++) {
 * if(pi.domEdgeRates[r][j]>5.0) System.out.println("Error");
 * 
 * } }
 * 
 * log.write("\nAcceptance:"); log.write( pi.domEdgeRates[0][0] + "\t" +
 * pi.domEdgeRates[0][1] + "\t" + pi.domEdgeRates[1][0] + "\t" +
 * pi.domEdgeRates[1][1] );
 * 
 * log.write("\nRejected:"); log.write( pi.domEdgeRates[0][0] + "\t" +
 * pi.domEdgeRates[0][1] + "\t" + pi.domEdgeRates[1][0] + "\t" +
 * pi.domEdgeRates[1][1] ); log.commit();
 * 
 * 
 * 
 * 
 ...
 * //////////////////////////////////////////////////// // import
 * phylogeny.NewickWriter; if(ps.level == Parameters.GENE_LEVEL && !
 * proposersList.get(0).getProposerName().equalsIgnoreCase("geneBDRates")) {
 * System.out.println("\n=======================================");
 * System.out.println( "Before perturbation at " + iteration + "\t" +
 * NewickWriter.getNewickString(pi.geneTree, pi.geneTree.vt,true) );
 * 
 * }
 * 
 * // In case of acceptance if(ps.level == Parameters.GENE_LEVEL && !
 * proposersList.get(0).getProposerName().equalsIgnoreCase("geneBDRates")) {
 * System.out.println( "Acceptance at" + "\t" + iteration + "\t" +
 * NewickWriter.getNewickString(pi.geneTree, pi.geneTree.vt,true) );
 * System.out.println("=======================================");
 * //System.out.println( "Acceptance at" + "\t" + iteration + "\t" +
 * oldStateLikelihood.show() + "\t" + "proposed_lk = " +
 * proposedStateLikelihood.show()); }
 * 
 * 
 * 
 * // In case of rejection if(ps.level == Parameters.GENE_LEVEL && !
 * proposersList.get(0).getProposerName().equalsIgnoreCase("geneBDRates")) {
 * System.out.println( "Rejection" + "\t" + oldStateLikelihood.show() + "\t" +
 * "proposed_lk = " + proposedStateLikelihood.show()); }
 * 
 * // After restoring if(ps.level == Parameters.GENE_LEVEL && !
 * proposersList.get(0).getProposerName().equalsIgnoreCase("geneBDRates")) {
 * System.out.println( "After restoring at     " + iteration + "\t" +
 * NewickWriter.getNewickString(pi.geneTree, pi.geneTree.vt,true) );
 * System.out.println("\n======================================="); }
 * 
 */
...
/*
 * if(ps.level == Parameters.GENE_LEVEL && !
 * proposersList.get(0).getProposerName().equalsIgnoreCase("geneBDRates")) {
 * //System.out.println(NewickWriter.getNewickString(pi.geneTree,
 * pi.geneTree.vt,true)); System.out.print(oldStateLikelihood.show());
 * System.out.println( "\t proposed_lk = " + proposedStateLikelihood.show() +
 * "\t" + "Rejection");
 * //System.out.println(NewickWriter.getNewickString(pi.geneTree,
 * pi.geneTree.vt,true) + "\t" + " *Rejection" + "\t" + iteration);
 ...
 * "\t" + "Acceptence");
 * System.out.println(NewickWriter.getNewickString(pi.geneTree,
 * pi.geneTree.vt,true) + "\t" + " *Acceptence" + "\t" + iteration);
 * 
 * }
 */

// System.out.println(NewickWriter.getNewickString(pi.geneTree,
// pi.geneTree.vt,true) + "\t" + "Time of root node =" +
// pi.geneTree.vt[pi.geneTree.root.id]);
// if(ps.level == Parameters.GENE_LEVEL && !
// proposersList.get(0).getProposerName().equalsIgnoreCase("geneBDRates")) {
// System.out.println(NewickWriter.getNewickString(pi.geneTree,
// pi.geneTree.vt,true));
...
/*
 * LogDouble a =
 * proposedStateLikelihood.overall.divToNew(oldStateLikelihood.overall); for
 * (Proposer proposer : proposersList) { if (!proposer.hasValidProposal()) {
 * doAccept = false; } a.mult( proposer.getDensityRatio() ); } // Decision of
 * Acceptance or Rejection LogDouble randomVal = new
 * LogDouble(prng.nextDouble()); if( a.greaterThanOrEquals(randomVal) ) doAccept
 * = true;
 */

/*
 * //double [] length1 =
 * Arrays.copyOf(pi.domainTree[0].bl,pi.domainTree[0].bl.length); //double []
 * length2 = Arrays.copyOf(pi.domainTree[1].bl,pi.domainTree[1].bl.length);
 * 
 * if(!doAccept) {
 * 
 * //pi.domainTree[0].bl = length1; //pi.domainTree[1].bl = length2;
 * 
 * for(int i=0 ; i<pi.domainTree[0].bl.length ; i++) if(pi.domainTree[0].bl[i]
 * != length1[i]) System.out.println("Error1");
 * 
 * for(int i=0 ; i<pi.domainTree[1].bl.length ; i++) if(pi.domainTree[1].bl[i]
 * != length2[i]) System.out.println("Error2");
 * 
 * }
 */
...
// System.out.println(iteration + "\t ratio = " + a.getLogValue() +"\t rv = " +
// randomVal.getLogValue());

// double totime = 0.0;
// long startTime, stopTime ;
// log.newLine();
// log.write("Approx. mean computational time for single state in (sec) = " + (
// totime / (double) TuningParameters.mcmcMaxIteration ) );
