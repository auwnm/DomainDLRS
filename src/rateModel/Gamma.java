...
/**
 * Not a gamma distribution per se; instead contains convenience methods
 * concerning gamma functions. Returning logarithms is often necessary to avoid
 * overflow. See also <code>GammaDistribution</code>.
 * 
...
public class Gamma {
	...
	// Ref. could be : http://mrob.com/pub/ries/lanczos-gamma.html
	/** 14 Lanczos coefficients for estimating ln(Gamma(xx)). */
	public static double[] LANCZOS_COEF = { 57.1562356658629235, -59.5979603554754912, 14.1360979747417471,
			-0.491913816097520199, .339946499848118887e-4, .465236289270485756e-4, -.983744753048795646e-4,
			.158088703224912494e-3, -.210264441724104883e-3, .217439618115212643e-3, -.164318106536763890e-3,
			.844182239838527433e-4, -.261908384015814087e-4, .368991826595316234e-5 };

	/**
	 * Computes the natural logarithm of the complete gamma function, i.e.,
	 * ln(Gamma(xx)) for xx > 0, where Gamma(x) = integral(t^(x-1) e^(-t),
	 * t=0,...,inf). Uses Lanczos' approximation formula, see Numerical Recipes 6.1.
	...
	/**
	 * Quantile function for the gamma distribution, i.e. the inverse of the CDF
	 * function F. Note that notation of scale parameter theta is such that
	 * mean=k*theta. Sometimes rate parameter beta=1/theta is used instead.
	 * <p/>
	...
	/**
	 * Computes the cumulative density function F(x) for a gamma distribution. Note
	 * that notation of scale parameter theta is such that mean=k*theta. Sometimes
	 * rate parameter beta=1/theta is used instead.
	 * 
	 * @param x
	 *            upper bound for region.
	 * @param k
	 *            shape parameter.
	 * @param theta
	 *            scale parameter
	...
	/**
	 * Computes the incomplete gamma ratio function for positive values of arguments
	 * x and alpha. Uses series expansion if alpha > x or x < 1.0, otherwise a
	 ...
	 * 
	 * @param x
	 *            the value up to which to integrate.
	 * @param k
	...
	/**
	 * Discretises a continuous gamma distribution into n "categories", each holding
	 * an equal part of the probability mass, 1/n, then returns the mean value r_i
	 * for each category i. Note: The mean is returned, not the mean density!
	 * 
	 * @param n
	 *            number of discrete categories that the gamma distribution will be
	 *            represented by.
	 * @param k
