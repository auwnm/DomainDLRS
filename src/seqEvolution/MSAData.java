...
/**
 * Handles multiple sequence alignment (MSA) data of a specified sequence type
 * (see <code>SequenceType</code>). The data is kept in a matrix with rows
 * corresponding to e.g. genes and columns corresponding to aligned positions of
 * these genes.
 * <p/>
 * The class provides methods for accessing the data and associated attributes,
 * and also for accessing a hash where a column pattern is the key and the
 * number of occurrences of this pattern is the value.
 * <p/>
 * User defined partitions of data (e.g. independent loci) are currently not
 * supported, see below. TODO: Bens: Add support for disjoint partitions of
 * sequence data. Joel: Perhaps that should be handled at a higher level? Seems
 * reasonable that an instance of this class represents one partition (loci).
 *
...
public class MSAData {
	...
	private int[][] data; // Sequence alignment matrix with states coded as ints. [i][j] for position j in
							// sequence i.
							// Ambiguity characters may be represented by states beyond the alphabet size.
	private SequenceType seqType; // Underlying sequence type.
	public String[] dataAsStrings; // Sequences as characters.
	public String[] leafOrderedSeqs; // Sequences as characters. (ordered w.r.t. leaves of the tree)
	private LinkedHashMap<String, Integer> nameToKey; // Name-to-index mapping for sequences.
	private int noOfPositions = -1; // Length of sequences.
	private LinkedHashMap<String, int[]> patterns; // Map where patterns (unique columns) are keys and [first position,
													// count] of the patterns are values.

	// Constructor initialises with Sequences
	public MSAData(SequenceType seqType, Sequences sequences) {

		int sz = sequences.nseqs;
		if (sz <= 0) {
		...
		this.leafOrderedSeqs = null;
		this.data = new int[sz][];

		int i = 0;
		for (Map.Entry<String, String> entry : sequences.seqMap.entrySet()) {
			this.addData(entry.getKey(), entry.getValue(), i);
			i++;
		}
	...
	/**
	 * Adds a sequence. All data is converted to lower case internally. Call
	 * updatePatterns() after adding all sequences.
	 * <p/>
	 * TODO: Bens: A short test conducted seemed to indicate that
	 * <code>SequenceData</code> distinguishes between capital and small letters in
	 * data - this is not good and need to be fixed!! Joel: This method now converts
	 * input data to lower case. Perhaps that solved it...?
	 * 
	 * @param name
	 ...
	 *            the sequence.
	 * @param seqIdx
	 *            the integer key of the sequence.
	 */
		...
		if (this.seqType == SequenceType.CODON) {
			...
			if (sz * 3 != sequence.length()) {
				throw new IllegalArgumentException(
						"Sequence " + name + " does not contain an even reading frame: length is not a multiple of 3.");
			}
		} else {
	...
	// This method will organize the sequences w.r.t leaf order.
	public void setLeafOrderedSequences(Tree tree) {

		this.leafOrderedSeqs = new String[this.getNoOfSequences()];

		for (int i = 0; i < tree.nnodes; i++) {
			if (tree.nodes.get(i).isLeaf()) {
				int idx = this.nameToKey.get(tree.nodes.get(i).name);
				this.leafOrderedSeqs[i] = this.dataAsStrings[idx];
			}
		}
	}

	public int[][] resampleAlign(int columns) {
		int[][] sMatrix = new int[this.getNoOfSequences()][columns];

		for (int j = 0; j < columns; j++) {

			// Randomly choose a column with replacement
			int sampleColumn = (int) (Math.random() * (this.getNoOfPositions()));

			// Copy column
			for (int i = 0; i < this.getNoOfSequences(); i++)
				sMatrix[i][j] = this.data[i][sampleColumn];
		}
		return sMatrix;
	}
	...
	/**
	 * Returns the number of positions (columns) in the data. A codon triplet counts
	 * as 1 position.
	 * 
	...
	/**
	 * Returns the index of a sequence. NOTE: This is NOT necessarily the same as
	 * the vertex number of a tree leaf corresponding to the sequence.
	 * 
	 * @param name
	 *            sequence identifier.
	 ...
	 * 
	 * @param seqIdx
	 *            the sequence index. This is NOT necessarily the same as the vertex
	 *            number of a tree leaf corresponding to the sequence.
	 * @param pos
	...
	/**
	 * Returns the likelihood of a character at a specific index of a specific
	 * sequence.
	 * 
	 * @param seqIdx
	 *            the sequence index. This is NOT necessarily the same as the vertex
	...
	/**
	 * Returns a specific sequence. Codons are coded using internal symbol
	 * representation.
	 * 
	 * @param name
	 *            the sequence identifier.
	...
	public String getInfoString() {
		...
		sb.append(this.seqType.toString()).append("; ").append("No. of sequences: ").append(this.nameToKey.size())
				.append("; ").append("No. of positions: ").append(this.noOfPositions).append('\n');
		return sb.toString();
	}
	 ...
	 * @return the names.
	 */
	public String[] getSequencesIdentifiers() {
		int i = 0;
		String[] names = new String[this.getNoOfSequences()];
		for (String name : this.nameToKey.keySet())
			names[i++] = name;
		return names;
	}

	/*
	 * public Set<String> getSequencesIdentifiersSet() { return
	 * this.nameToKey.keySet(); }
	 */
